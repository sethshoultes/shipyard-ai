 ...
 * ai-ts.ts
 *
 * Generates src/ai.ts with a simple abstraction layer for AI services:
 * - Primary: Workers AI (@cf/meta/llama-2-7b-chat-int8)
 * - Fallback: Anthropic Claude (if ANTHROPIC_API_KEY is provided)
 *
 * REQ-021, REQ-022, REQ-023, REQ-024, REQ-025, REQ-026 compliance:
 * - Simple chat() function with auto-routing
 * - Graceful error handling with clear messages
 * - Automatic fallback on quota exceeded or rate limit errors
 * - Clear error messages with actionable setup URLs
 */

export interface AiTsConfig {
  // Currently no configuration needed - always generates the same pattern
}

/**
 * Generate the src/ai.ts file with Workers AI + Claude fallback abstraction
 *
 * @returns TypeScript source code for ai.ts
 */
export function generateAiTs(_config?: AiTsConfig): string {
  return `/**
...
/**
 * Validates the generated ai.ts structure
 * Checks for: required functions, error handling, documentation
 *
 * @param aiTs The generated ai.ts content
 * @returns { isValid: boolean, errors: string[] }
 */
export function validateAiTs(aiTs: string): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];

  // Check for required exports
  if (!aiTs.includes('export const ai =')) {
    errors.push('Missing required export: ai');
  }

  if (!aiTs.includes('function chat(')) {
    errors.push('Missing required function: chat');
  }

  if (!aiTs.includes('function claudeChat(')) {
    errors.push('Missing required function: claudeChat');
  }

  // Check for key error messages
  if (!aiTs.includes('quota_exceeded')) {
    errors.push('Missing quota_exceeded error handling');
  }

  if (!aiTs.includes('ANTHROPIC_API_KEY')) {
    errors.push('Missing ANTHROPIC_API_KEY reference');
  }

  if (!aiTs.includes('@cf/meta/llama-2-7b-chat-int8')) {
    errors.push('Missing Workers AI model specification');
  }

  // Check for clear error messages with URLs
  if (!aiTs.includes('console.anthropic.com/account/keys')) {
    errors.push('Missing Anthropic console URL in error message');
  }

  // Check for documentation
  if (!aiTs.includes('/**')) {
    errors.push('Missing JSDoc comments');
  }

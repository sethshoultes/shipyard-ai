...
async function fireWebhook(
	...
	const logs = (await ctx.kv.get<string[]>(`webhook:${endpoint.id}:logs`)) ?? [];
	logs.push(log.id);
	// Keep last 100 logs
...
async function getMemberByStripeCustomerId(
	...
	try {
		...
		const memberEmails = (await ctx.kv.get<string[]>("members:list")) ?? [];
...
async function sendWelcomeEmail(
	...
	try {
		...
		const plans = (await ctx.kv.get<PlanConfig[]>("plans")) ?? DEFAULT_PLANS;
		...
		register: {
			...
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					if (!selectedPlan) {
						throw new Error("We couldn't find that plan — it may have been removed");
					}

					// Check existing member
					const encodedEmail = emailToKvKey(email);
					const existingMember = await ctx.kv.get<MemberRecord>(`member:${encodedEmail}`);
					if (existingMember) {
						return {
							memberId: email,
					...
					if (existingLock) {
						...
						throw new Error("Hold on — we're still processing your previous request");
					}
					...
					if (selectedPlan.price > 0) {
						...
						const membersList = (await ctx.kv.get<string[]>(listKey)) ?? [];
			...
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					if (!stripeApiKey) {
						...
						throw new Error("Payment processing isn't set up yet");
					}
					...
					// Reject free plans
					if (selectedPlan.price <= 0) {
						throw new Error("Free plans don't require checkout");
					}

					// Look up or create member first
					const encodedEmail = emailToKvKey(email);
					let member = await ctx.kv.get<MemberRecord>(`member:${encodedEmail}`);
					...
					if (!stripeCustomerId) {
						...
						if (!customerRes.ok) {
							...
							throw new Error("We couldn't set up your payment account — please try again");
						}
					...
					if (!checkoutRes.ok) {
						...
						throw new Error("We couldn't start the checkout — please try again");
					}
			...
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					if (!sessionRes.ok) {
						...
						throw new Error("We couldn't find that checkout session");
					}
					...
					if (!customerEmail) {
						throw new Error("We couldn't find your email in the checkout session");
					}

					if (!subscription) {
						throw new Error("We couldn't find the subscription in the checkout session");
					}
					...
					if (!subscriptionId) {
						throw new Error("We couldn't find a valid subscription");
					}
					...
					if (!member) {
						...
						const checkoutData = await ctx.kv.get<any>(`checkout:${sessionId}`);
		...
		dashboard: {
			...
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					const jwtSecret = (ctx as any).env?.JWT_SECRET as string | undefined;
					if (!jwtSecret) {
						throw new Error("Authentication isn't set up yet");
					}
		...
		dashboardCancel: {
			...
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					if (!token) {
						throw new Error("Please sign in to cancel your subscription");
					}
		...
		dashboardUpgrade: {
			...
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					if (!currentPlan) {
						throw new Error("We couldn't find your current plan");
					}
		...
		couponCreate: {
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					// Add to coupons list
					const coupons = (await ctx.kv.get<string[]>("coupons:list")) ?? [];
		...
		couponList: {
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					const couponCodes = (await ctx.kv.get<string[]>("coupons:list")) ?? [];

					const coupons: CouponRecord[] = [];
					for (const code of couponCodes) {
						const coupon = await ctx.kv.get<CouponRecord>(`coupon:${code}`);
						if (coupon) {
							coupons.push(coupon);
						}
					}
						...
						try {
							if (action === "approve") {
								...
								await ctx.kv.set(
									`member:${encodedEmail}`,
									{
										...(existingMember || { email, plan: "", createdAt: new Date().toISOString() }),
										status: "active",
			...
			portal: {
				...
				handler: async (routeCtx: unknown, ctx: PluginContext) => {
					try {
						...
						const ruleIds = (await ctx.kv.get<string[]>("gating-rules:list")) ?? [];
						const accessibleIds: string[] = [];

						for (const ruleId of ruleIds) {
							const rule = await ctx.kv.get<any>(`gating-rule:${ruleId}`);
			...
			checkGatingAccess: {
				...
				handler: async (routeCtx: unknown, ctx: PluginContext) => {
					try {
						...
						let rule = null;
						for (const ruleId of ruleIds) {
							const r = await ctx.kv.get<any>(`gating-rule:${ruleId}`);
		...
		groupCreate: {
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					// Add to groups list
					const groups = (await ctx.kv.get<string[]>("groups:list")) ?? [];
					groups.push(groupId);
		...
		groupInvite: {
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					// Get group
					const group = await ctx.kv.get<GroupRecord>(`group:${groupId}`);
					if (!group) {
						throw new Error("We couldn't find that group");
					}
					...
					const invites = (await ctx.kv.get<string[]>(`group:${groupId}:invites`)) ?? [];
					invites.push(code);
		...
		groupAccept: {
			...
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					// Get invite
					const invite = await ctx.kv.get<GroupInviteCode>(`group:invite:${code}`);
					if (!invite) {
					...
					// Get group
					const group = await ctx.kv.get<GroupRecord>(`group:${invite.groupId}`);
					if (!group) {
						throw new Error("We couldn't find that group");
					}
			...
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					if (!url || !events || events.length === 0) {
						throw new Error("Please provide a URL and at least one event");
					}
					...
					const webhooks = (await ctx.kv.get<string[]>("webhooks:list")) ?? [];
		...
		webhookDelete: {
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					if (!webhookJson) {
						throw new Error("We couldn't find that webhook");
					}
		...
		webhookList: {
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					const webhookIds = (await ctx.kv.get<string[]>("webhooks:list")) ?? [];

					const webhooks: WebhookEndpoint[] = [];
					for (const id of webhookIds) {
						const webhook = await ctx.kv.get<WebhookEndpoint>(`webhook:${id}`);
						if (webhook) {
		...
		webhookTest: {
			handler: async (routeCtx: unknown, ctx: PluginContext) => {
				try {
					...
					const webhook = await ctx.kv.get<WebhookEndpoint>(`webhook:${webhookId}`);
					if (!webhook) {
						throw new Error("We couldn't find that webhook");
					}

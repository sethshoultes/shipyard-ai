 ...
 * Show progress bar
 */
function showProgress(percent, label = 'Downloading') {
    const width = 30;
    const filled = Math.round((width * percent) / 100);
    const empty = width - filled;
    const bar = '█'.repeat(filled) + '░'.repeat(empty);
    process.stdout.write(`\r${label}... [${bar}] ${Math.round(percent)}%`);
}
/**
...
async function validateTheme(themeName) {
    ...
    if (!theme) {
        throw new Error(`Theme "${themeName}" not found. Available themes: ${themes
            .map((t) => t.name)
            .join(', ')}`);
    }
    return theme.name;
 ...
 */
async function getThemeDetails(themeName) {
    const themes = await fetchThemesRegistry();
 ...
 */
export async function installTheme(themeName, options = {}) {
    ...
    try {
        ...
        try {
            ...
            if (!hasRequiredFiles) {
                throw new Error('Theme is missing required files: live.config.ts or pages/index.astro');
            }
 ...
 */
function findExtractedSrc(basePath) {

 ...
 */
export async function rollback(name, versionArg, options) {
    // Load config - error if not initialized
    const config = loadConfig();
    if (!config) {
        throw new Error("Not initialized. Run 'tuned init' first to create .tuned.json");
    }
    try {
        ...
        if (versionArg) {
            ...
            const previousVersion = sortedVersions.find((v) => v.version < activeVersion.version);
            if (!previousVersion) {
        ...
        if (!options?.yes) {
            const confirmed = await promptConfirm(`Roll back ${name} to v${targetVersion}? [y/N] `);
            if (!confirmed) {

 ...
 */
function formatPromptTable(
  prompts: Array<{ name: string; active_version: number; updated_at: string }>
): string {
  if (prompts.length === 0) {
  ...
  const nameWidth = Math.max(
    "NAME".length,
    ...prompts.map((p: { name: string; active_version: number; updated_at: string }) => p.name.length)
  );
  ...
  // Format rows
  const rows = prompts.map((p: { name: string; active_version: number; updated_at: string }) => {
 ...
 */
function formatVersionTable(
  versions: Array<{ version: number; is_active: boolean; created_at: string }>
): string {
...
export const listCommand = new Command()
  ...
  .action(async (name: string | undefined, options: { json?: boolean }) => {
    try {
      ...
      if (!config) {
        console.error(
          'Error: No .tuned.json configuration found. Run "tuned init" first.'
        );
        process.exit(1);
      }

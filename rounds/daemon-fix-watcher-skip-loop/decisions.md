...
- **Opposed by:** Elon
- **Winner:** Elon
- **Why:** The essence decrees: "No rename until the queue is real." Operational drag during a P0 patch breaks runbooks, scripts, and muscle memory for zero user-facing value. Rebrand is a v2 conversation. Steve concedes the tactical patch ships today; the poetry waits.

### 2. Filesystem queue + mtime check ships in v1.
- **Proposed by:** Steve (ship the tourniquet now)
- **Opposed by:** Elon (SQLite single-table is the real fix; ship architecture now)
- **Winner:** Steve on scope, Elon on record.
- **Why:** A strategic SQLite rewrite is v2. The team agreed: ship the band-aid to stop the bleeding, but document the architectural debt on merge so the next engineer knows the real target. The mtime check is accepted as a tactical patch — not an atomic, idempotent, or race-safe solution.
...
- **Opposed by:** None
- **Winner:** Elon
- **Why:** If you need to change exports to test 30 lines of pure logic, you're testing wrong. Use a temp directory and mock the constants. Changing the module surface area to chase coverage is a v2 feature masquerading as v1.
...
- **Opposed by:** None
- **Winner:** Elon
- **Why:** A `TODO` comment is not enough. If the strategic fix does not have a tracked ticket before the tactical patch merges, the band-aid becomes permanent by default. "Documented debt" means a ticket in the backlog with a trigger threshold, not a comment in the source code.
...
- **Why:** The first 30 seconds decide whether engineers actually run the daemon. Elon concedes the friction must drop, but the fix is a shell script and clean, grep-friendly logs — not a brand-voice overhaul or renaming.
...
- **Why:** The CLI output should be legible and the log format grep-friendly. Steve's brand voice is accepted for info-level lifecycle events. But if a failure is final, the log must say "failed" plainly. Confidence is good; denial dressed as optimism is not.
...
- **Why:** Internal directories (`prds/failed/`) exist for 2 a.m. debugging (Elon wins on operational reality), but they are not surfaced in user-facing onboarding, CLI help, or primary UX (Steve wins on product). The interface is logs and scripts; directories are the emergency escape hatch, not the storefront.
...
7. **Clean log formatting** — grep-friendly, plain language on fatal errors, no passive-obfuscation of real failures.
...
3. **Retry ceiling** — How many intake attempts before dead-letter? 1? 3? Configurable? (Steve: no knobs; Elon: boundaries are hygiene. Need a number.)
4. **Recovery UX** — Manual `mv` back to intake, or a `scripts/retry.sh` helper?
5. **v2 migration trigger** — What metric trips the SQLite rewrite? 500 PRDs/day? 3rd filesystem race? Document the threshold in code comments and the P1 ticket.
...
| **Dead-letter spam** | Medium | Low | Build agent | Without retry ceiling, a bad PRD could loop forever. Set hard ceiling in v1 (Open Question #3). |

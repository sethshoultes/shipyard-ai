...
A single blank line between heading and table would let the eye settle before parsing structured data.
...
One uses hardcoded paths, the other uses environment variable fallbacks. Both are valid patterns. Using both is a decision not made.

**Comment headers vary in capitalization:**
...
### 2. Consolidate the Log Function

Create `nerve/lib/log.sh` containing the shared `log()` function. Source it from each script. The current repetition is defensible but the consolidation would be more honest — these scripts already source each other (see `daemon.sh` line 66: `source "$(dirname "$0")/queue.sh"`).

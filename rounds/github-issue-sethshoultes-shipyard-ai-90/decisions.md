...
|---|----------|-------------|--------|-----------|
| 1 | **Name: Relay** | Steve (Round 1: "We're calling it Relay. One word. Two syllables.") | Steve; Elon conceded Round 2 | Elon: "Relay is better than AgentForge." One syllable would be Forge, but Relay is the promise Steve fought for and Elon accepted. Essence anchors on the feeling; Relay is the name that carries it. |
| 2 | **Brand voice: human, confident, zero acronyms, no enterprise sludge** | Steve (Round 1: "We speak like the smartest person in the room who actually respects your time.") | Steve; Elon conceded Round 2 | Elon agreed: "No jargon, no dark patterns, no desperate upsell badges — correct." Warmth is a feature. Humanity is the moat. |
| 3 | **Engine before baton — execution model defined before pixels** | Elon (Round 1: "The hard part is execution semantics, not pretty boxes." Round 2: "State machine defined in code before a single pixel is placed.") | Elon; Steve conceded Round 2 | Steve: "He's right that 'workflow' is undefined under the hood. We need durable state and idempotency before the first pixel ships." Essence commands: "Engine first. Baton later." |
| 4 | **No drag-and-drop canvas for v1; config UI first** | Elon (Round 1: "Ship a config UI, not Figma. Canvas is a v2 feature masquerading as v1." Round 2: "No canvas for v1. Config forms and a JSON editor. Canvas is a roadmap item, not a launch item.") | Elon | Steve defended canvas as core interaction (Round 2: "The canvas ships in v1. Not read-only. Not YAML. Canvas."). But Elon's compound-interest argument holds: a JSON config UI ships in 48 hours; a canvas ships in 6 weeks or never. The engine is the product; the UI is documentation. Essence: "No canvas v1." |
| 5 | **One platform only: web app** | Elon (Round 1: "Pick ONE. Dual-platform splits focus and doubles QA.") + Steve (Round 1: "NO to the WordPress plugin.") | Unanimous | Steve Round 2: "He is absolutely right that SaaS-only is the call." A standalone web app with no traffic source is dangerous, but bifurcation is mediocrity. WordPress is a wrapper, not a launchpad. |
...
| 9 | **Hard token budgets + per-user cost caps day one** | Elon (Round 1: "per-user cost caps. One user running workflows on a cron could cost $500/month.") | Elon; Steve conceded | Steve Round 2: "Elon was right: we cap the spend before we uncap the wonder." Essence: "Hard cost caps. No bankruptcy." COGS kills startups faster than churn. |
| 10 | **One free workflow; no freemium billing stack in v1** | Synthesis of Elon (paid-only urgency) + Steve (free tier for learning) + Essence ("One free workflow.") | Phil's tiebreak | Steve wants a free tier ("We monetize limits and power users, not access"). Elon wants paid-only from day one. The Zen path: one capped trial workflow, no Stripe, no metered billing complexity in v1. Hard caps prevent bankruptcy regardless. |
...
| 12 | **No dark mode; one aesthetic: white, airy, optimistic** | Steve (Round 1: "NO to dark mode as default — light, airy, optimistic.") + Essence ("White. Airy. No dark mode.") | Steve; uncontested | The app is a baton, not a toolbox. Power users who need dark mode can use the API. |
| 13 | **No admin dashboard with 40 toggles** | Steve (Round 1: "NO to admin dashboards with 40 toggles.") | Unanimous | "If you need forty toggles, you haven't decided what the product is." One knob when possible, zero when not. |
...
### Core Experience
1. **Relay web app** — single-platform, no WordPress bifurcation
